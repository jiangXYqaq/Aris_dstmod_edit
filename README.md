...
**槽位占用:** 1  BETA
- 装备后自动为爱丽丝 （`alice`），物品栏和手持的可充电物品充电
- **可充电物品：**
  - 爱丽丝手机 (`alice_remote`)
  - 电池 (`alice_battery`) 
  - 原版可充电物品，如晨星（`night_stick`）

### 2. 强化魔法 (Magic Enhancement)
`Prefab: wx78module_alc_magic`  
**槽位占用:** 2  ALPHA
- **属性增益：**
  - 生命值+240 / 饱食度+100 / 精神值+200 / 护盾上限+最大生命值的50%
  - 饱食消耗速度降低50%（每日仅消耗37.5）
  - 每5秒回复：10生命值/满血改为等量护盾值 + 20精神值 + 0.5护盾值
- 恒温：永远保持舒适温度
- 战斗强化：增加暴击率与暴击伤害，增加30%物理伤害减免
- 安魂: 受疯狂光环效果降低为原来的25%，装备物品的理智增益增加50%
...
- **属性：**
  ...
  - 100%防水 + 25%移速加成
  - 精神回复+6.67/分钟 + 火焰免疫
- **容器：** 内置8格容器（2×4），末格为防护板专用插槽，其余格子可存放任意物品
- **防腐：** 盐盒保鲜效果
...
- **属性：**
  ...
  - +20位面防御
  - 免疫酸雨/月雹
- **容器：** 内置14格容器（2×7），末格为防护板专用插槽，其余格子可存放任意物品
- **防腐：** 锡鱼罐反鲜效果
