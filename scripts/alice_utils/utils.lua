...
---Alice utils
function FN.FindEquipWithTag(owner, tag, container)
    if not (owner or tag) then
        return
    end
    
    local inventory = TheWorld.ismastersim and owner.components.inventory or owner.replica.inventory
    local weapon = inventory and inventory:GetEquippedItem(EQUIPSLOTS.HANDS) -- 从装备栏找
    if weapon and weapon:HasTag(tag) then
        return weapon
    end

    local allinventory = TheWorld.ismastersim and inventory.itemslots or inventory:GetItems()
    for k, v in pairs(allinventory) do -- 从物品栏找
    ...
    if container then
        ...
        if backpack then
            for k, v in pairs(backpack:GetItems()) do -- 从背包找
                if v:HasTag(tag) then
                    return v
                end
            end
